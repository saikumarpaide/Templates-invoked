...
import { webhookOnImportPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [webhookOnImportPlugin],
    });

    await request(server).get('/api/webhook-on-import/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/webhook-on-import/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/webhook-on-import/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/webhook-on-import/todos/${createdTodoItem.id}`)
